...
#define NLINE_MAX 50
// name of config file expected in program directory at run time
#define CONFIG_FILE_NAME "config.file"

// enum type to specify host type
enum host_type {SERVER, CLIENT};

//a linked list to store server address and ports read in from config file
struct address{
    char* ip;
    int port;
};

// a linked list node to store server address (IP and port number)
struct address_node{
    struct address *addr;
    struct address_node *next;
};

struct address_node *init_address_list(void);
void add_address_node(struct address_node **address_list, struct address addr);
...
// extracts ip string and port number from a line read from the config file
void parse_address_line(char **ip, int *port, const char *config_line);
