...
int main(int argc, char *argv[]){
    ...
    struct list *broadcast_list = init_list(sizeof(struct list));
    ...
    while(1){
        ...
        int rc = select (socket_fd+1, &socketFDS, NULL, NULL, &timeout); // NEW block until something arrives
        ...
        if (FD_ISSET(STDIN, &socketFDS)){
            ...
            if(strlen(buffer) == 0){
                ...
                send_data(send_socket_fd, buffer, config_list, 
                            out_history_list, broadcast_list, self_addr->location, port);
            }
            continue;
        }
            ...
            if(kv_list){
                ...
                if(msg_mov_kv){
                    ...
                    if(valid_port(kv_list, port)){
                        ...
                        struct node *msg_node = broadcast_list->head;

                        while(msg_node){
                            struct list *msg = (struct list *)msg_node->data;
                            retransmit_data(send_socket_fd, config_list, msg, broadcast_list, self_addr->location);
                ...
                // Wrong Port AND in range AND TTL>0
                    ...
                    // Add message to cache only if it does not already exist
                    if(!(struct list *)find(broadcast_list, kv_list, is_equal_message)){
                        struct pair *msg_send_path_kv = (struct pair *)find(kv_list, "send-path", is_key_match);
                        char *path = malloc(sizeof(char)*50);
                        sprintf(path, "%s,%d", msg_send_path_kv->value, self_addr->port);
                        msg_send_path_kv->value = path;
                        append(broadcast_list, kv_list);
                    }
