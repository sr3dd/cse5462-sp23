...
char *message_constructor(const char *og_msg, int seqNumber, 
								int location, int port, int ttl, int version){
    char append_str[100];
    sprintf(append_str, " seqNumber:%d location:%d fromPort:%d TTL:%d send-path:%d version:%d", 
						seqNumber, location, port, ttl, port, version);
...
char *move_message_constructor(const char *og_msg, int port, int ttl, int version){
	char append_str[100];
    sprintf(append_str, " fromPort:%d TTL:%d version:%d", 
						port, ttl, version);
...
bool is_equal_message(void *node_data, void *data){
	...
	struct list *node = (struct list *)node_data;
	struct list *msg = (struct list *)data;

	for(int i = 0; i < (int)(sizeof(MESSAGE_SIMILARITY_KEYS)/sizeof(char *)); i++){
        struct pair *kv_pair = (struct pair *)find(msg, MESSAGE_SIMILARITY_KEYS[i], is_key_match);
		struct pair *node_kv_pair = (struct pair *)find(node, MESSAGE_SIMILARITY_KEYS[i], is_key_match);
		
		if(node_kv_pair && kv_pair){
			if(strcmp(kv_pair->value, node_kv_pair->value) != 0){
				return false;
			}
		}
		else{
